...
class EnhancedChangelogGenerator:
    ...
    def __init__(self, model: str = "gpt-5.5", language: str = "en"):
        self.model = model
    ...
    def _create_enhanced_context(
        ...
        return {
            ...
            "main_components": [
                comp for comp, count in stats["most_changed_components"]
            ],
            "component_impact": component_impact_serializable,
            "commits_by_type": {k: len(v) for k, v in commits_by_type.items()},
            "date_range": (
                f"{stats['date_range'][0].strftime('%Y-%m-%d')} to {stats['date_range'][1].strftime('%Y-%m-%d')}"
                if stats["date_range"]
                else None
            ),
        }
    ...
    def process_commits_intelligently(
        ...
        def _summarize_commit_analysis() -> str:
            summary: List[str] = []
            summary.append(f"Version impact: {version_impact[0]} ({version_impact[1]})")

            if grouped_commits:
                summary.append("Grouped highlights:")
                for key, group in list(grouped_commits.items())[:8]:
        ...
        commits_template = resolve_template_path(
            "commits_prompt", self.language, enhanced=True
        )
    ...
    def determine_smart_version(
        ...
        version_template = resolve_template_path(
            "version_prompt", self.language, enhanced=True
        )
    ...
    def generate_enhanced_changelog(
        ...
        changelog_template = resolve_template_path(
            "changelog_prompt", self.language, enhanced=True
        )
    ...
    def _post_process_changelog(
        ...
        raw_lines = re.sub(
            r"^[-*]\s+", "- ", changelog, flags=re.MULTILINE
        ).splitlines()
        ...
        sections = {
            title: bullets for title, bullets in sections.items() if any(bullets)
        }
        ...
        def _build_auto_summary() -> List[str]:
            ...
            breaking = _fmt_count(
                int(context.get("breaking_changes") or 0), "breaking change"
            )
            if breaking:
...
def analyze_changelog_quality(changelog: str) -> Dict[str, Any]:
    ...
    for idx, ln in enumerate(lines[1:], start=1):  # after header
        if ln.startswith("### "):
            ...
            if current is None:
                ...
                headings.append(current)
                bullets_by_heading.setdefault(current, [])
            bullets_by_heading[current].append(
                re.sub(r"\s+", " ", ln[2:].strip().lower())
            )
    ...
    quality_metrics = {
        "has_proper_header": bool(
            re.search(r"## \[.*\] - \d{4}-\d{2}-\d{2}", changelog)
        ),
    ...
    if bullets:
        quality_metrics["avg_bullet_length"] = sum(
            len(bullet) for bullet in bullets
        ) / len(bullets)
