    ...
    def _infer_commit_type(self, message: str, files_changed: List[str]) -> str:
        ...
        if files_changed:
            if any(f.endswith((".md", ".rst", ".txt")) for f in files_changed):
                return "docs"
            if any(
                "test" in f.lower() or f.endswith((".test.js", ".spec.js", "_test.py"))
    ...
    def get_enhanced_commits(
        ...
        try:
            for commit in self.repo.iter_commits(
                f"{from_ref}..{to_ref}", no_merges=True
            ):
...
def format_commits_for_ai(commits: List[CommitInfo], include_stats: bool = True) -> str:
    ...
    if include_stats:
        ...
        output.append(
            f"Code changes: +{stats['total_insertions']} -{stats['total_deletions']}"
        )

        if stats["most_changed_components"]:
            components = [
                f"{comp}({count})" for comp, count in stats["most_changed_components"]
            ]
